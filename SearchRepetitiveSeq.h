#pragma once//используем поле dictionary для поиска подпосл
...
public:
	...
	Dictionary<std::string, int>* PreficsDict(Sequence<char>* seq, int Lmin, int Lmax)//возвращает словарь подпосл / реализация обычного алгоритма поиска подпосл / дикшн исп для исключения повторения подпосл
	{
		int maxCount(1);
		std::string maxStr("");
